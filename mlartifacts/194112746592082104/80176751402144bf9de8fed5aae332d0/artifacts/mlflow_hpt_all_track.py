...
grid_search = GridSearchCV(estimator= gb, param_grid= param_grid, cv= 5, n_jobs= -1, verbose= True)

mlflow.set_experiment('diabetes-gbc-hypertune-all track')

with mlflow.start_run(description = 'Best hyperparameter trained GBC model') as parent:
    ...
    mlflow.sklearn.log_model(grid_search.best_estimator_, "gradient boosting classifier", signature = signature)
